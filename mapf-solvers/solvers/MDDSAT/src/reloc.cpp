...
/*----------------------------------------------------------------------------*/
	...
	if (!m_Neighbors.empty())
	{
	    ...
	    for (Neighbors_list::const_iterator neighbor = m_Neighbors.begin(); neighbor != m_Neighbors.end(); ++neighbor)
	    {
		fprintf(fw, "%d->%d ", (*neighbor)->m_target->m_id, (*neighbor)->m_source->m_id);
	    }
	    fprintf(fw, "}");
...
// sEdge
	...
	if (m_directed)
	{
	    fprintf(fw, "%s Edge  %d: %d --> %d\n", indent.c_str(), m_id, m_arc_uv.m_source->m_id, m_arc_uv.m_target->m_id);
...
/*----------------------------------------------------------------------------*/
    ...
    void sUndirectedGraph::generate_Network(int *Matrix, int size, int x_size, int y_size, int min_aisle_length, int max_aisle_length)
    {
	Edges_vector Edges;
    ...
    int sUndirectedGraph::calc_GridVerexID(int grid_row, int grid_column) const
    {
	sASSERT(m_Matrix != NULL);
...
/*----------------------------------------------------------------------------*/
	...
	for (Edges_list::const_iterator edge = m_Edges.begin(); edge != m_Edges.end(); ++edge)
	{
	    fprintf(fw, "%s{%d,%d} (-1)\n", indent.c_str(), edge->m_arc_uv.m_target->m_id, edge->m_arc_vu.m_target->m_id);
	}
    }
