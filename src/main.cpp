...
std::string algorithmOne(const std::string& mapFilePath, const std::string& scenFilePath) {
    // --- CBSH ---
    // You can easily change your heuristics and other parameters right here!
    
    std::string chosenHeuristic = "WDG";         // Options: "Zero", "CG", "DG", "WDG"
    std::string chosenRectReasoning = "GR";      // Options: "None", "R", "RM", "GR", "Disjoint"
    std::string chosenCorridorReasoning = "GC";  // Options: "None", "C", "PC", "STC", "GC", "Disjoint"
    ...
    if (cbs.solution_found) {
        return "CBSH Result: Success! Solved in " + std::to_string(runtime) + " seconds.";
    } else {
        return "CBSH Result: Failed to find a solution within " + std::to_string(cutoffTime) + " seconds.";
    }
}
...
std::string algorithmTwo(const std::string& input) {
...
void readMapAndScenFiles(const std::string& mapFilePath, const std::string& scenFilePath) {
    std::ifstream mapStream(mapFilePath);
    std::ifstream scenStream(scenFilePath);

    // 1. Process the .map file
    if (mapStream.is_open()) {
        std::cout << "Successfully opened .map file: " << mapFilePath << "\n";
        std::string line;
        ...
        std::cerr << "Error: Could not open .map file at " << mapFilePath << "\n";
    }

    // 2. Process the .scen file
    if (scenStream.is_open()) {
        std::cout << "Successfully opened .scen file: " << scenFilePath << "\n";
        std::string line;
        
        // Read line by line
        while (std::getline(scenStream, line)) {
            // TODO: Parse the scenario data 
        }
        scenStream.close();
    } else {
        std::cerr << "Error: Could not open .scen file at " << scenFilePath << "\n";
    }
}
...
int main() {
    std::cout << "--- Testing String Functions & CBSH ---\n";
    
    // Replace these strings with your actual local file paths
    std::string mapFile = "test_grid.map";
    std::string scenFile = "test_grid.scen";
    std::string myInput = "PathfindingTest";
    
    // Now algorithmOne takes the two file paths!
    std::cout << algorithmOne(mapFile, scenFile) << "\n";
    
    std::cout << algorithmTwo(myInput) << "\n";
    std::cout << algorithmThree(myInput) << "\n";
    std::cout << algorithmFour(myInput) << "\n";
