...
std::vector<mapf::Agent> sample_n(const std::vector<mapf::Agent> &agents,
                                  std::size_t n);
...
void scip_tmp_copy(const std::filesystem::path &f1,
                   const std::filesystem::path &f2);

void scip_tmp_clear();

void benchmark_problem(MapfasterValuation &valuator,
  ...
  for (const auto &scenario_path : problem.scenarios) {
    ...
    const std::vector<mapf::Agent> scenario =
        mapf::reader::read_scenario(scenario_path);
    ...
    auto val_elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                              val_end - val_start)
                              .count();
    ...
    std::thread t_cbs([&]() {
      ...
      mapf::Solution solved =
          cbs_solve(problem.map, scenario_path, scenario.size());
    ...
    std::thread t_cbsh([&]() {
      ...
      mapf::Solution solved =
          cbsh_solve(problem.map, scenario_path, scenario.size());
    ...
    std::thread t_bcp([&]() {
      ...
      mapf::Solution solved = bcp_solve(
          60, std::filesystem::absolute(tmp / scenario_path.filename()));
    ...
    auto threads_elapsed_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(threads_end -
                                                              threads_start)
            .count();
...
void benchmark_problem_serial(MapfasterValuation &valuator,
                              const ManifestProblem &problem) {
  ...
  for (const auto &scenario_path : problem.scenarios) {
    ...
    // Start timer for ALL solvers combined

    auto solvers_start = std::chrono::steady_clock::now();
    ...
    // End timer for ALL solvers and output to file

    auto solvers_end = std::chrono::steady_clock::now();

    auto solvers_elapsed_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(solvers_end -
                                                              solvers_start)
            .count();

    out << "ALL took " << solvers_elapsed_ms << " ms" << std::endl;
  }
...
int main() {
  ...
  for (int i = 0; i < 1; ++i) {
    ...
    benchmark_problem_serial(valuator, problems[0]);
  }

  for (const auto &problem : problems) {

    benchmark_problem_serial(valuator, problem);
  }
}
...
void scip_tmp_copy(const std::filesystem::path &f1,
  ...
  std::filesystem::copy_file(f1, tmp / f1.filename(),
                             std::filesystem::copy_options::overwrite_existing);

  std::filesystem::copy_file(f2, tmp / f2.filename(),
                             std::filesystem::copy_options::overwrite_existing);
}
...
std::vector<mapf::Agent> sample_n(const std::vector<mapf::Agent> &agents,
  ...
  std::ranges::sample(agents, std::back_inserter(result), n,
...
std::unordered_map<mapf::Agent, std::vector<mapf::Pos>> find_minimal_paths(
  ...
  for (const mapf::Agent &agent : agents) {
    ...
    if (grid.is_blocked(start.row, start.col) ||
        grid.is_blocked(goal.row, goal.col)) {
